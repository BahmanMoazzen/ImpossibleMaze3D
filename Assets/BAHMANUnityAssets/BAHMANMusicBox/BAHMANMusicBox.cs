...
public class BAHMANMusicBox : MonoBehaviour
{
    ...
    {
        StopAllCoroutines();
        StartCoroutine(_sceneLoaded(iLoadedScene.name));

    }
    IEnumerator _sceneLoaded(string iSceneName)
    {
        //find current scene setting
        int sceneMusicIndex = INVALID_INDEX_NUMBER;
        for (byte i = 0; i < _musicBoxSceneInfos.Length; i++)
        {
            if (_musicBoxSceneInfos[i]._SceneName.Equals(iSceneName))
            {
    ...
    public void _ChangePlayingStat([Tooltip("True = Start Play; False=Stop Play")] bool iPlayingStat)
    {
        if (iPlayingStat)
        {
            StartCoroutine(_sceneLoaded(SceneManager.GetActiveScene().name));
        }
        else
        {
            StopAllCoroutines();
